...
export default function GlyphGrid({
  ...
  // Ref for high-frequency pointer updates
  ...
  const handlePointerMove = (e) => {
    ...
    if (nodeIdx !== -1) {
      ...
      if (lastNode !== nodeIdx) {
        ...
        setActiveNodes(prev => [...prev, nodeIdx]);
        
        let a = lastNode;
        let b = nodeIdx;
        if (a > b) { let tmp = a; a = b; b = tmp; }
        
        // Prevent duplicate edges if we want (optional, but standard ingress allows backtracking)
        // Actually Ingress usually records edges once. Let's just record it.
        setDrawnEdges(prev => {
  ...
  const handlePointerUp = (e) => {
    ...
    if (onInputEnd) {
      ...
      // Sort edges for canonical representation? Wait, Ingress canonical edge representation:
      // Sort each edge so a < b, then sort all edges.
      ...
      >
        ...
        {isDrawing && activeNodes.length > 0 && (
          <line
            ...
            x2={NODE_POS[activeNodes[activeNodes.length - 1]][0]} 
            y2={NODE_POS[activeNodes[activeNodes.length - 1]][1]}
